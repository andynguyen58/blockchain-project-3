    ...
    RetailerRole
{
    ...
    function sellItem(
        uint256 _upc,
        uint256 _price,
        address payable _distributorID
    ) public onlyFarmer packed(_upc) verifyCaller(items[_upc].originFarmerID) {
        // Update the appropriate fields
        items[_upc].itemState = State.ForSale;
    ...
    // Call modifier to check if upc has passed previous supply chain stage
    // Call modifier to verify caller of this function
    function shipItem(uint256 _upc, address payable retailerID)
        public
        onlyDistributor
        sold(_upc)
    ...
    function receiveItem(uint256 _upc, address payable consumerID)
        public
        onlyRetailer
        shipped(_upc)
    {
